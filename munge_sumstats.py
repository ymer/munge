...
parser.add_argument('--N-con', default=None, type=float,
                    ...
                    "column, and this flag is set, the argument to this flag has priority.")
parser.add_argument('out', default=None, type=str, nargs='?',
...
parser.add_argument('--chunksize', default=5e4, type=int,
                    help="Chunksize.")
parser.add_argument('--merge-arch', default=None, type=str,
                    help="Merge-arch file is used to update SNPs names to newest versions before merging.")
parser.add_argument('--a1-inc', default=False, action='store_true',
                    help="A1 is the increasing allele.")
parser.add_argument('--round-N', default=False, action='store_true',
                    help="Round the N values down to nearest integer")
parser.add_argument('--round-vals', default=False, action='store_true',
                    help="Round the p and OR values to three digits")
parser.add_argument('--rem-mhc', default=False, action='store_true',
                    help="Removes the MCH region (assumens build 37)")
parser.add_argument('--compress', default=False, action='store_true',
                    help="Gzip the output file")
parser.add_argument('--use-or', default=False, action='store_true',
...
parser.add_argument('--nstudy-min', default=None, type=float,
                    help="Minimum # of studies. Default is to remove everything below the max, unless there is an N "
                    "column, in which case do nothing.")
...
def parse_dat(dat_gen, convert_colname, merge_alleles, args):
    ...
    dat_list = []
    if args.merge_arch:
        logging.info('Reading MergeArch from {F}'.format(F=args.merge_arch))
        arch = pd.read_csv('files/RsMergeArch.bcp', sep='\t', header=None, usecols=[0,1], dtype=str, names=['OLD', 'NEW'])
        arch.OLD = 'rs' + arch.OLD
        arch.NEW = 'rs' + arch.NEW
    else:
        arch = None
    ...
    for block_num, dat in enumerate(dat_gen):
        ...
        if 'FRQ' in dat.columns:
            if set(dat['FRQ']) == {-9}:
                msg = 'WARNING: All frq values are -9.'
                logging.info(msg)
            else:
    ...
    if args.merge_alleles or args.merge_bim:
        msg += 'Removed {N} SNPs not in --merge-alleles ({percent} percent).\n'.format(
            N=drops['MERGE'], percent=print_percent(drops['MERGE'], tot_snps))

    msg += 'Removed {N} SNPs with missing values.\n'.format(N=drops['NA'])
    msg += 'Removed {N} SNPs with INFO <= {I} ({percent} percent).\n'.format(
        N=drops['INFO'], I=args.info_min, percent=print_percent(drops['INFO'], tot_snps))
...
def process_n(dat, args):
    ...
    if 'N' in dat.columns:
        ...
        new = len(dat)
        logging.info('Removed {M} SNPs with N < {MIN} ({N} SNPs remain) ({percent} percent removed).'.format(
            M=old - new, N=new, MIN=n_min, percent=print_percent((old-new), old)))
        ...
        new = len(dat)
        logging.info('Removed {M} SNPs with NSTUDY < {MIN} ({N} SNPs remain) ({percent} percent removed).'.format(
            M=old - new, N=new, MIN=nstudy_min, percent=print_percent((old-new), old)))
...
def allele_merge(dat, alleles, args):
    """
...
def munge_sumstats(args, p=True):
    ...
    try:
        ...
        if args.daner:
            frq_u = [n for n in file_cnames if n.startswith("FRQ_U")][0]
            frq_a = [n for n in file_cnames if n.startswith("FRQ_A")][0]
            N_cas = float(frq_a[6:])
            N_con = float(frq_u[6:])
            logging.info(
        ...
        if args.merge_alleles or args.merge_bim:
            if args.merge_bim:
                logging.info('Reading list of SNPs for allele merge from {F}'.format(F=args.merge_bim))
                merge_alleles = pd.read_csv(args.merge_bim, header=None, names=['CHR', 'SNP', 'GPOS', 'POS', 'A1', 'A2'], sep='\s+')
            else:
                logging.info('Reading list of SNPs for allele merge from {F}'.format(F=args.merge_alleles))
        ...
        if signed_sumstat_null == 1:
            dat = dat.rename(columns={'SIGNED_SUMSTAT': 'OR'})
        else:
            if args.use_or:
                sign_cname = 'OR'
                dat['OR'] = dat.SIGNED_SUMSTAT.apply(lambda x: math.e**x)

        # do this last so we don't have to worry about NA values in the rest of
        # the program
        if args.rem_mhc:
            datcols = dat.columns
            if not 'POS' in dat.columns:
                dat = pd.merge(dat, merge_alleles[['SNP', 'CHR', 'POS']], on='SNP', how='inner')
            old = len(dat)
            dat = dat.loc[~((dat.CHR == 6) & (dat.POS < 33448354) & (dat.POS > 28477797))]
            new = len(dat)
            dat = dat[datcols]
            logging.info('Removed {M} SNPs in the MHC region ({N} SNPs remain).'.format(
                M=old - new, N=new))

        if args.merge_alleles:
            dat = allele_merge(dat, merge_alleles, args)

        out_fname = args.out + '.sumstats'
        print_colnames = [
            c for c in dat.columns if c in ['SNP', 'CHR', 'POS', 'N', 'A1', 'A2', 'P', 'OR', 'FRQ', 'SE']]
        dat = dat.dropna(subset=['Z'])
        logging.info("Writing summary statistics for {A} SNPs to {B}.".format(A=len(dat), B=out_fname))
        ...
        if args.hess:
            ...
            for chrom in range(1, 23):
                ...
                if not datchrom.empty:
                    datchrom = datchrom[['rsID', 'pos', 'A0', 'A1', 'Z-score', 'N']]
        ...
        logging.info('{N} Genome-wide significant SNPs (some may have been '
                'removed by filtering).'.format(N=(CHISQ > 29).sum()))
        return dat
